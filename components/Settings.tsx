               ...
               >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                     ...
                     <div className="bg-slate-900/40 border border-white/5 p-6 rounded-[2rem]">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Quote Studio Defaults</h4>
                        <div className="grid grid-cols-1 gap-4">
                           <div>
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Default Margin</label>
                              <div className="mt-2 relative">
                                 <input
                                    type="number"
                                    value={settings.quoteDefaultMargin}
                                    onChange={(e) => updateSetting('quoteDefaultMargin', Number(e.target.value))}
                                    className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-white font-mono font-bold text-lg outline-none"
                                 />
                           ...
                           <div>
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Rounding</label>
                              <div className="mt-2 flex gap-2">
                                 {(['none', '5', '10'] as const).map(r => (
                                    <button
                                       key={r}
                                       onClick={() => updateSetting('quoteRounding', r as any)}
                                       className={`px-3 py-2 rounded-lg text-[12px] font-bold ${settings.quoteRounding === r ? 'bg-white/10 text-white' : 'bg-slate-800 text-slate-400'}`}
                           ...
                           <div>
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Template ID</label>
                              <input
                                 type="text"
                                 value={settings.quoteTemplateId || ''}
                                 onChange={(e) => updateSetting('quoteTemplateId', e.target.value || null)}
                                 className="w-full mt-2 bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-white outline-none"
                              />
                           </div>

                           <div className="flex items-center justify-between">
                              <div>
                                 <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Deep Analysis</p>
                                 <p className="text-[11px] text-slate-400">Enable detailed pricing reasoning in generated quotes.</p>
                              </div>
                              <button
                                 onClick={() => updateSetting('quoteDeepAnalysis', !settings.quoteDeepAnalysis)}
                                 className={`px-4 py-2 rounded-lg ${settings.quoteDeepAnalysis ? 'bg-emerald-500 text-white' : 'bg-slate-800 text-slate-400'}`}
                              >{settings.quoteDeepAnalysis ? 'On' : 'Off'}</button>
                           </div>
                        </div>
                     </div>
